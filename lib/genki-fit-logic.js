...
// Вътрешни нива на Product Price Support за Genki Fit.
//
// 100% е ПРЕМАХНАТО (решение 2026-09-23). Причината е продуктова, не
// техническа: при 100% служителят вече практически не участва финансово
// в покупката и това става друг, изцяло финансиран модел на потребление.
// Genki Fit не бива да предлага такъв модел автоматично.
//
// Таванът на автоматичната препоръка е 75%. Ако бюджетът стига за
// повече, препоръката пак спира на 75% — излишъкът НЕ се тълкува като
// 100% субсидия.
export const PS_LEVELS = [10, 20, 30, 50, 75];
export const PS_LEVELS_AUTOMATIC = [75, 50, 30, 20, 10];   // отгоре надолу
export const PS_MAX_AUTOMATIC = 75;
 ...
 *
 * Последният праг е t75, не t100: горната лента означава „над нужното за
 * 75%", а НЕ „100% подкрепа".
 */
export function psThresholds(ceiling) {
  return {
    t20: round50(ceiling * 0.20),
    t50: round50(ceiling * 0.50),
    t75: round50(ceiling * 0.75),
  };
}
...
export function budgetBands(q2, q3, q5) {
  ...
  const psShapedBands = () => withIds(
    contiguousBands([0, thresholds.t20, thresholds.t50, thresholds.t75], 0),
  ...
  const bands = withIds(
    contiguousBands(
      [base - 1, base + thresholds.t20 - 1, base + thresholds.t50 - 1, base + thresholds.t75 - 1],
      0
    ),
...
/* ==========================================================================
   ...
   Genki Fit кодът се генерира на СЪРВЪРА, при успешно изпращане, и живее
   в lib/genki-fit-email.js. Клиентът не измисля идентификатори.

   Предишният клиентски GENKI_FIT_ID при старт е премахнат: нищо не се
   съхранява при старт, така че той нямаше нито един консуматор. Бриф
   раздел 21 иска уникален идентификатор за подаването — това е кодът.
   ========================================================================== */

/* ==========================================================================
   11. ПРОГРЕСИВНА ВАЛИДАЦИЯ

   Проверява само това, което дадената стъпка ВЕЧЕ трябва да знае.
   Зависимостите се пазят и тук: Q3 се сверява срещу Q2, а Q6 — срещу
   лентите, които сървърът сам генерира от Q2/Q3/Q5.

   Полета, които стъпката още не покрива, се отсяват — така връщането
   назад и смяната на Q2 наистина изчистват зависимите Q3 и Q6, вместо
   да оставят сесията вътрешно противоречива.
   ========================================================================== */

export const MAX_STEP = 6;

export function validateProgressive(raw, step) {
  const errors = [];
  const a = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {};
  const n = Number(step);

  if (!Number.isInteger(n) || n < 1 || n > MAX_STEP) {
    return { ok: false, errors: ['step'], answers: null, step: null };
  }

  /* --- Q1 --- */
  const cities = Array.isArray(a.cities) ? a.cities.filter((c) => Q1_CITIES.includes(c)) : [];
  if (!cities.length) errors.push('q1');
  ...
  /* --- Q2 --- */
  const q2 = n >= 2 ? (Q2_BANDS.includes(a.q2) ? a.q2 : null) : null;
  if (n >= 2 && !q2) errors.push('q2');

  const largestOffice = q2 && Q2_BANDS.includes(a.largestOffice) ? a.largestOffice : null;

  /* --- Q3 --- */
  let q3 = null;
  if (n >= 3) {
    const allowed = q2 ? Q3_RANGES_BY_Q2[q2] : [];
    q3 = q2 && allowed.includes(a.q3) ? a.q3 : null;
    if (!q3) errors.push('q3');
  }
  ...
  let q4 = [];
  if (n >= 4) {
    const picked = Array.isArray(a.q4) ? a.q4.filter((v) => Q4_OPTIONS.includes(v)) : [];
    if (!picked.length) errors.push('q4');
    q4 = picked.includes('none') ? ['none'] : picked;
  }

  /* --- Q5 --- */
  const q5 = n >= 5 ? (Q5_OPTIONS.includes(a.q5) ? a.q5 : null) : null;
  if (n >= 5 && !q5) errors.push('q5');
